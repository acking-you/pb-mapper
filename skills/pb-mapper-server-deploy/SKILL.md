...
description: Deploy or upgrade the unified `pb-mapper` binary on a remote Linux host, run its `server` command as a managed `systemd` service, and verify the relay is listening. Use when users ask to set up the central pb-mapper relay through SSH.
---
...
echo "Installed: $(${INSTALL_DIR}/pb-mapper --version)"
REMOTE_INSTALL
```
