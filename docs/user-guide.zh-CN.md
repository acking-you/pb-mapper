...
- `--auth-state-dir`：认证状态目录，默认 `/var/lib/pb-mapper/auth`
...
中继首次启动时会在 `/var/lib/pb-mapper/auth/admin.key` 生成随机管理员密钥，系统不再
提供内置默认 key。管理员密钥留在中继机器上，用它为业务签发临时凭据：
...
- `PB_MAPPER_AUTH_STATE_DIR`：中继认证状态目录，默认 `/var/lib/pb-mapper/auth`
