...
// LoadIconWithScaleDown scales far better than LoadImage when the .ico has no
// entry at the requested size, but it only exists in comctl32 v6. Flutter's
// runner manifest does not declare a dependency on Common-Controls 6, so the
// process gets v5.82 from System32, where the export is absent, and linking
// against it statically makes the plugin fail to load at all. Resolve it at
// runtime and fall back where it is missing.
using LoadIconWithScaleDownPtr = HRESULT(WINAPI*)(HINSTANCE, PCWSTR, int, int,
                                                  HICON*);

LoadIconWithScaleDownPtr GetLoadIconWithScaleDown() {
  static LoadIconWithScaleDownPtr cached = [] {
    // Plain LoadLibrary rather than a System32-pinned load, so that an app
    // whose manifest does activate v6 gets the better implementation.
    HMODULE comctl = LoadLibraryW(L"comctl32.dll");
    return comctl ? reinterpret_cast<LoadIconWithScaleDownPtr>(
                        GetProcAddress(comctl, "LoadIconWithScaleDown"))
                  : nullptr;
  }();
  return cached;
}
...
class TrayManagerPlugin : public flutter::Plugin {
 ...
 private:
  ...
  // Kept so the icon can be re-rasterised at the new size when the DPI
  // changes; the shell hands us one bitmap, so the only way to stay sharp is
  // to load a fresh one.
  ...
  void TrayManagerPlugin::_ReloadIcon();
...
std::optional<LRESULT> TrayManagerPlugin::HandleWindowProc(HWND hWnd,
  ...
  if (message == WM_DESTROY) {
    ...
    // The shell keeps the single bitmap it was handed, so a scaling change
    // leaves the tray upscaling the old one. Reload at the new size instead.
    if (tray_icon_setted) {
      _ReloadIcon();
...
void TrayManagerPlugin::SetIcon(
  ...
  icon_path = converter.from_bytes(iconPath);
...
// Rasterises the icon at the size the tray wants on the monitor the window is
// currently on.
//
// GetSystemMetrics is not per-monitor: on a mixed-DPI desktop it answers for
// the primary display, so the icon comes out sized for the wrong monitor.
// GetSystemMetricsForDpi asks the question properly. Both it and
// GetDpiForWindow need Windows 10 1607, which is below Flutter's own floor.
//
// LoadIconWithScaleDown is preferred over LoadImage because when the .ico has
// no entry at the requested size, LoadImage falls back to a GDI stretch with
// no filtering and the result is visibly soft. It is not always available
// (see GetLoadIconWithScaleDown), and LoadImage is exact anyway whenever the
// file does carry the requested size.
void TrayManagerPlugin::_ReloadIcon() {
  if (icon_path.empty()) {
    return;
  }

  UINT dpi = GetDpiForWindow(GetMainWindow());
  if (dpi == 0) {
  ...
  LoadIconWithScaleDownPtr scale_down = GetLoadIconWithScaleDown();
  if (scale_down != nullptr &&
      FAILED(scale_down(nullptr, icon_path.c_str(), cx, cy, &loaded))) {
    loaded = nullptr;
  }
  if (loaded == nullptr) {
    loaded = static_cast<HICON>(LoadImage(nullptr, icon_path.c_str(),
                                          IMAGE_ICON, cx, cy,
                                          LR_LOADFROMFILE));
  }

  // Only drop the old handle once the new one is in hand; a failed load
  // otherwise leaves the tray with no icon at all.
  if (loaded == nullptr) {
    return;
  }
